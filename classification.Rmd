...
start_time <- Sys.time()
factor_combinations_n <- foreach(n = seq(2, ncol(feature_matrix)),
...
factor_combinations_n_ratio = factor_combinations_n %>%
  mutate(bsstssRatio = map(km,
                           ~.x$betweenss/.x$totss) %>%
           as.numeric()) %>%
  arrange(desc(bsstssRatio))

for (i in 1:20) {
  factor_combinations_n_ratio %>%
    slice(i) %>%
    pull(km_plot) %>%
    print()
}
...
```{r}
kmeans_clusters <- factor_combinations_n_ratio %>%
  slice(1) %>%
  pull(km) %>%
  pluck(1) %>%
  pluck("cluster") %>%
...
cluster_summary <- full_streaming_history %>%
  ...
  mutate(month = ts %>%
           ...
           paste0("-01") %>%
           ymd(),
         track_artist = paste(master_metadata_track_name,
                              master_metadata_album_artist_name,
                              sep = " - ")) %>%
  select(month,
         track_artist,
         ms_played) %>%
  left_join(kmeans_clusters, by = "track_artist") %>%
